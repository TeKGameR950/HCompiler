...
namespace RoslynCSharp.Compiler
{
    public class RoslynCSharpCompiler
    {
        ...
        public CompilationResult CompileFromSyntaxTree(Microsoft.CodeAnalysis.SyntaxTree[] syntaxTrees, IMetadataReferenceProvider[] additionalAsemblyReferences)
        {
            ...
            if (RoslynCSharp.settings.generateInMemory == false && parent.Exists == false)
                parent.Create();

            // Create output stream
            Stream outputStream = (RoslynCSharp.settings.generateInMemory == true)
                ? (Stream)new MemoryStream()
                : (Stream)new FileStream(outputPath, FileMode.OpenOrCreate, FileAccess.ReadWrite);

            Stream outputPDBStream = null;

            if (RoslynCSharp.settings.generateSymbols == true)
            {
                string outputPDBPath = Path.ChangeExtension(outputPath, outputPDBExtension);

                outputPDBStream = (RoslynCSharp.settings.generateInMemory == true)
                    ? (Stream)new MemoryStream()
        ...
        private void UpdateParserOptions()
        {
            ...
            parseOptions = new CSharpParseOptions(
                RoslynCSharp.settings.languageVersion,
        ...
        {
            OptimizationLevel optimizeLevel = (RoslynCSharp.settings.allowOptimizeCode == true)
            ...
            compileOptions = new CSharpCompilationOptions(
                ...
                RoslynCSharp.settings.allowUnsafeCode,                                                    // Allow unsafe
                null,                                                           // Krypto key container
                null,                                                           // Krypto key file
                default(System.Collections.Immutable.ImmutableArray<byte>),     // Krypto public key
                null,                                                           // Delay sign
                RoslynCSharp.settings.targetPlatform,                                                 // Platform
                ReportDiagnostic.Info,                                       // Diagnostic option
                RoslynCSharp.settings.warningLevel,                                                   // Warning level
                null,                                                           // Specific diagnostic options
                RoslynCSharp.settings.allowConcurrentCompile,                                         // Allow concurrent compile
                RoslynCSharp.settings.deterministic,                                                          // Deterministic
